...
async function snapshotDirectoryChain(path) {
  const snapshots = [];
  for (const current of directoryChain(path)) {
...
async function assertDirectoryChainUnchanged(snapshots) {
...
export async function atomicWrite(path, content, { overwrite = false } = {}) {
  ...
  const original = await optionalLstat(path);
...
export async function writeMemoryCore(records, outputDirectory, { onFileWritten = null } = {}) {
  ...
  try {
    ...
    if (await optionalLstat(outputDirectory) !== null) {
