...
export async function writeMemoryCore(records, outputDirectory) {
  ...
  for (const name of Object.keys(files).sort()) {
    ...
    try {
      await stat(join(outputDirectory, name));
      throw new Error(`refusing to overwrite existing rendered file: ${join(outputDirectory, name)}`);
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    }
  }
  for (const name of Object.keys(files).sort()) {
    await atomicWrite(join(outputDirectory, name), files[name]);
  }
