...
function recordFromMapping(item, source, path, index) {
  ...
  return createMemoryRecord({
    ...
    scope: item.scope || item.namespace || item.agentId || "default",
...
export async function loadRecords(adapter, paths) {
  ...
  for (const path of paths) {
    ...
    else result.push(...await readJsonExport(path, adapter));
  }
  return result;
}
